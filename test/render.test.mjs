...
test('no detail page contains an unescaped data-driven angle bracket in body text', () => {
  for (const t of trailers) {
    ...
    // application/ld+json BreadcrumbList block, plus up to 9 application/json
    // data islands (tow-data, fuel-data, payload-data, finance-data,
    // ownership-data, cost-night-data, trip-cost-data, water-calc-data,
    // propane-data, elec-data) depending on trailer specs.
    // Minimum 5 (theme + app.js + ld+json product + ld+json breadcrumb + tow-data), maximum 14 (all tools).
    assert.ok(scripts.length >= 5 && scripts.length <= 14, `${t.slug} has unexpected <script> count: ${scripts.length}`);
