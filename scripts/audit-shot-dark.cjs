// Dark-theme screenshot: set localStorage ae:theme=dark on the origin first.
// Usage: node audit-shot-dark.cjs <url> <outpath.png> <width> <height> [waitMs]
const net=require('net'),http=require('http'),crypto=require('crypto'),fs=require('fs');
const URL_=process.argv[2],OUT=process.argv[3],W=+(process.argv[4]||1280),H=+(process.argv[5]||900),WAIT=+(process.argv[6]||1500);
...
// Navigate to origin once to set localStorage, then to the target.
const origin=new URL(URL_).origin;
await cdp.send('Page.navigate',{url:origin+'/index.html'});await sleep(700);
await cdp.send('Runtime.evaluate',{expression:`try{localStorage.setItem('ae:theme','dark');}catch(e){}`,returnByValue:true});
await cdp.send('Page.navigate',{url:URL_});await sleep(WAIT);
await cdp.send('Runtime.evaluate',{expression:`(async()=>{document.querySelectorAll('img[loading=lazy]').forEach(i=>i.loading='eager');const h=document.body.scrollHeight;for(let y=0;y<h;y+=600){window.scrollTo(0,y);await new Promise(r=>setTimeout(r,50));}window.scrollTo(0,0);try{await Promise.all([...document.images].filter(i=>i.src).map(i=>i.decode().catch(()=>{})));}catch(e){}try{await document.fonts.ready;}catch(e){}})()`,returnByValue:true,awaitPromise:true});
await sleep(600);
