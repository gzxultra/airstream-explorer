// Forced-LIGHT screenshot: clear ae:theme and emulate prefers-color-scheme:light.
// Usage: node audit-shot-light.cjs <url> <out.png> <w> <h> [waitMs]
const net=require('net'),http=require('http'),crypto=require('crypto'),fs=require('fs');
const URL_=process.argv[2],OUT=process.argv[3],W=+(process.argv[4]||1280),H=+(process.argv[5]||900),WAIT=+(process.argv[6]||1600);
...
await cdp.send('Runtime.evaluate',{expression:`try{localStorage.removeItem('ae:theme');}catch(e){}`,returnByValue:true});
